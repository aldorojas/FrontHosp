...
function verEncuentros(idPaciente){
    ...
    getDataEncuentrosPaciente(1, idPaciente )
...
divScrollModal[0].addEventListener('scroll', () => {
    ...
    if (st > lastScrollTop){
        if (divScrollModal[0].scrollTop + divScrollModal[0].clientHeight >= divScrollModal[0].scrollHeight) {        
            ...
            setTimeout(() => {
                loader.classList.add('hidden');
                getDataEncuentrosPaciente(page);
            }, 2000);
        }
    }
...
const getDataEncuentrosPaciente = async (page_no = 1, paramSearch) => {
    const data = await httpRequestWrapper(
    "GET",
    'http://134.122.120.195/api/v1/encuentros_per_patient?patient='+ paramSearch +'&entry_n=' + `${page_no}`
    );
    if (data[0]== 0){
        const Toast = Swal.mixin({
            toast: true,
